...
三種跑法都實跑過：乾淨匯出、uv 安裝與零金鑰 HTTP 問答在本機；`docker compose -f docker-compose.demo.yml up --build`
2026-09-06 在一台 Ubuntu Docker 主機上從零 build 起來，`/health` 200、Replay 問答有回。
...
代價很誠實：回放只認得錄過的那八句（見第 5 節）。問別的會拿到
...
- **示範資料的「現在」是固定的**（錨點 `2026-09-01`）。好處是可重現，
  代價是每個查詢都要明講 `as_of`。
- **Replay 模式只重播錄好的那八句**。問錄音以外的問題要自己帶金鑰（跑法 b）。
...
上表是**帶金鑰的十題實跑**，不是 Replay 的分數。八段出貨錄音只有部分問句
